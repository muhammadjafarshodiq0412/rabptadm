...
class Proyek_model extends CI_Model
{
   ...
   function cek1($id_proyek) {
        $this->db->where("id_proyek", $id_proyek);
        //$this->db->where("password", $password);
        return $this->db->get("detail_proyek");
      }
   ...
   function __construct()
     ...
     public function getSingleBua($field, $value)
    {
     ...
     public function getSingleProyek($field, $value)
    {
        $this->db->where($field, $value);
   ...
   function get_all()
    ...
    {
        $query = "select proyek.*, user.* FROM proyek join user ON proyek.user_id=user.user_id order by id_proyek DESC";
        return $this->db->query($query)->result_array();
    }
     public function getAHS()
    {
        $query = "select * FROM ahs";
        return $this->db->query($query)->result_array();
    }

     public function getBua_By_id_proyek($field, $value)
    {
        $query ="select detail_proyek.*, proyek.* FROM detail_proyek join proyek ON detail_proyek.id_proyek=proyek.id_proyek where detail_proyek.id_proyek = '$field, $value'";
        return $this->db->query($query)->result_array();
    }   
     public function getSingleProyek_by_id($id)
    {
       $this->db->select('t1.*, t2.*')
     ->from('detail_proyek as t1')
     ->where('t1.id_proyek', $id)
     ->join('proyek as t2', 't1.id_proyek = t2.id_proyek', 'LEFT')
   //  ->join('table3 as t3', 't1.id = t3.id', 'LEFT')
    ...
    public function updateDetail_proyek($id, $post)
    {
        $this->db->where('id_proyek', $id);
        $this->db->where('id_bua', $id);
        $this->db->update('detail_proyek', $post);
